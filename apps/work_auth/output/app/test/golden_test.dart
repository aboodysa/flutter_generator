...
void main() {
  ...
  testWidgets('WorkAuthListScreen renders empty with CTA (golden)', (tester) async {
    setupDependencies();
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.reset);
    await tester.pumpWidget(BlocProvider<WorkAuthListCubit>(
      create: (_) => sl<WorkAuthListCubit>(),
      child: MaterialApp(theme: buildTheme(), home: WorkAuthListScreen()),
    ));
    await tester.pumpAndSettle();
    await expectLater(find.byType(WorkAuthListScreen), matchesGoldenFile('goldens/work_auth_list_screen_empty.png'));
  });
}
