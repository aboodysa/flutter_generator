...
void main() {
  ...
  testWidgets('WorkAuthWizardScreen: first step wires a focus-bypass FocusNode on every text-input field', (tester) async {
    setupDependencies();
    await tester.pumpWidget(const ReplicaApp());
    await tester.pumpAndSettle();
    appRouter.go('/work-auth/wizard');
    await tester.pumpAndSettle();
    expect(find.byType(TextFormField), findsNWidgets(4), reason: 'the wizard first step must render exactly its own text-input widgets');
    ...
    await tester.pump();
    expect(fields[1].focusNode!.hasFocus, isTrue, reason: 'tapping wizard text-input field 1 must actually transition it to focused');
    await tester.tap(find.byType(TextField).at(2));
    await tester.pump();
    expect(fields[2].focusNode!.hasFocus, isTrue, reason: 'tapping wizard text-input field 2 must actually transition it to focused');
    await tester.tap(find.byType(TextField).at(3));
    await tester.pump();
    expect(fields[3].focusNode!.hasFocus, isTrue, reason: 'tapping wizard text-input field 3 must actually transition it to focused');
  });
}
