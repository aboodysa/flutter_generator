...
void main() {
  ...
  testWidgets('QuizRun: create form wires a focus-bypass FocusNode on every text-input field', (tester) async {
    setupDependencies();
  ...
  testWidgets('QuizRun: edit form wires a focus-bypass FocusNode on every text-input field', (tester) async {
    setupDependencies();
    await tester.pumpWidget(const ReplicaApp());
    await tester.pumpAndSettle();
    appRouter.go('/quiz-run/x/edit');
    await tester.pumpAndSettle();
    final fields = tester.widgetList<TextField>(find.byType(TextField)).toList();
    final bypassFields = fields.where((f) => f.focusNode != null).toList();
    expect(bypassFields.length, 1, reason: 'every text-input field (String/int/double/Money) needs its own FocusNode bypass, and DateTime must stay readOnly with none');
