...
void main() {
  ...
  test('a repo update on an already-exported ExpenseClaim row throws (immutability)', () async {
    final repo = ExpenseClaimRepositoryInMemoryImpl();
    final items = await repo.listExpenseClaims();
    final original = items.first;
    final exported = ExpenseClaim(id: original.id, name: original.name, amount: original.amount, status: original.status, exported: true);
    await repo.updateExpenseClaim(exported); // not-yet-exported -> exported: allowed
    expect(repo.updateExpenseClaim(exported), throwsA(isA<StateError>()), reason: 'already-exported rows are immutable — corrections require void + clone');
  });
}
