...
void main() {
  ...
  test('a repo createLeaveRequest() write-ahead-enqueues an OutboxMessage before returning', () async {
    final repo = LeaveRequestRepositoryInMemoryImpl();
    final before = Outbox.instance.pending().length;
    await repo.createLeaveRequest(LeaveRequest(id: 'leave-request-1', name: 'Sample LeaveRequest 1', leaveType: LeaveType.values.first, startDate: DateTime(2025), endDate: DateTime(2025), days: 1, status: LeaveStatus.values.first, reason: 'Sample item 1', exported: false));
    ...
    expect(pending.last.entity, 'LeaveRequest');
