/**
 * OutboxCoreGenerator — structural, deterministic, 0% LLM (MF6).
 * Emitted once per app with `attributes.outbox: true` (operations.ts's hasOutbox). Mirrors
 * core/audit.dart's shape exactly: an immutable message value object + an app-wide, in-memory,
 * singleton queue — entity-agnostic (payload is a generic `Map<String, dynamic>?` snapshot built
 * at the call site via `${Entity}Model.fromEntity(e).toJson()`, same as L3's audit before/after).
 *
 * This is the client-side half of what P9's real backend will later drain (a sync loop posting
 * `pending()` messages to the generated NestJS API and marking them sent/failed) — that replay
 * mechanism is explicitly out of scope here (no network, no real sync); this slice only makes
 * the queue itself real, deterministic, and testable.
 */
export function generateOutboxCore(): string {
  return `// [generated] generator=OutboxCoreGenerator template=outbox_core.v1 class=structural ownership=generated
