/**
 * AuditCoreGenerator — structural, deterministic, 0% LLM (L3).
 * Emits `core/audit.dart` once per app that opts >=1 entity into `audited: true`
 * (operations.ts's hasAudit). Mirrors L2/MF4/MF5's precedent: a single shared, app-type-agnostic
 * module — AuditEvent/AuditLog depend only on generic `Map<String, dynamic>` snapshots, never on
 * any entity-specific field, so the same file works unchanged whether it's Ledgerly's ExpenseClaim
 * or hr_service's LeaveRequest being audited.
 *
 * `before`/`after` are `${Entity}Model.fromEntity(e).toJson()` snapshots built at the call site
 * (repository_impl.ts) — this file stays entity-agnostic by never importing a specific entity;
 * the caller does the entity-specific conversion, this file only stores/renders the result.
 */
export function generateAuditCore(): string {
  return `// [generated] generator=AuditCoreGenerator template=audit_core.v1 class=structural ownership=generated
