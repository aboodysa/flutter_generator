...
export function runPipeline(ir: any, outDir: string, irPath: string): PipelineReport {
  ...
  const vPass = v.determinism && v.headers + v.secrets + v.idioms + v.arch + v.oracle + v.fidelity === 0;
  stages.push({ stage: "validators", status: vPass ? "pass" : "fail", detail: v.issues.join("; ") || "determinism + secrets + headers + idioms + arch + oracle + fidelity all pass" });
