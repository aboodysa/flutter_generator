...
import 'package:rasheed_replica_expense_tracker/core/router.dart';
...
void main() {
  ...
  testWidgets('Transaction: create form wires a focus-bypass FocusNode on its first field', (tester) async {
    setupDependencies();
    await tester.pumpWidget(const ReplicaApp());
    await tester.pumpAndSettle();
    appRouter.go('/transaction/new');
    await tester.pumpAndSettle();
  ...
  testWidgets('Transaction: edit form wires a focus-bypass FocusNode on its first field', (tester) async {
    setupDependencies();
    await tester.pumpWidget(const ReplicaApp());
    await tester.pumpAndSettle();
    appRouter.go('/transaction/x/edit');
    await tester.pumpAndSettle();
    final field = tester.widget<TextField>(find.byType(TextField).first);
    expect(field.focusNode, isNotNull, reason: 'first field needs an explicit FocusNode for the gesture-bound requestFocus bypass (iOS Safari keyboard fix)');
    expect(field.onTap, isNotNull, reason: 'onTap must call requestFocus() synchronously inside the tap gesture');
