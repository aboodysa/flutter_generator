...
- [~] CDP: demo login → capture(stub) → submit → manager approve → budget shows remaining → CSV — login/submit/budget/CSV confirmed live in-browser; capture(stub) has no UI entry point (by design, LM7); manager approve now has a real UI entry point and was CDP re-verified live (approve + reject both flip the row, correct color/icon, sibling rows untouched) — see `apps/ledgerly/output/qa/cdp-acceptance/LM6-approve-reject-rerun.md`. Still `[~]` not `[x]` only because capture(stub) has no entry point by design (LM7), not a gap
