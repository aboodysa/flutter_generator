#!/usr/bin/env bash
# run_loop.sh — objective-driven autonomous execution harness. Reads an objective file shaped like
# core/objective.md.template (goal / check_cmd / lane / from / until_commit / level / max_rounds /
# timeout_mins / prompt), loops dispatch -> poll -> verify against a tmux lane until the objective
# is met, and reports progress via report.sh. Execution logic lives here; verbosity lives in
# report.sh — changing ORCH_PROGRESS_LEVEL must never change what this script *does*.
#
# The framework is project-agnostic: the objective file carries the repo-specific check_cmd,
# prompt and lane; --repo selects which git repo to verify against (default: the git root of the
# current working directory). Only tmux (for --lane dispatch/poll) and git are required.
#
# Usage:
...
repo="${opt_repo:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
